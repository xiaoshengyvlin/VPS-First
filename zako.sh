...
# ============ 配置: TCP 优化 ============
config_tcp() {
    print_title "TCP 缓冲区优化"
    echo "  扩大 TCP 缓冲区，优化高延迟/跨国线路下的传输性能。"
    echo "  适合代理/VPN/VLESS 等场景。"
    echo ""
    if ask_yn "是否开启 TCP 优化?"; then DO_TCP=true; print_info "将开启 TCP 优化"
    else DO_TCP=false; print_info "跳过 TCP 优化"; fi
}

do_tcp() {
    print_info "开启 TCP 缓冲区优化..."
    _tcp_ok=0 _tcp_failed=0

    _apply_tcp() {
        _tcp_key="$1" _tcp_val="$2"
        grep -q "^${_tcp_key}" /etc/sysctl.conf 2>/dev/null || echo "${_tcp_key} = ${_tcp_val}" >> /etc/sysctl.conf
        sysctl -w "${_tcp_key}=${_tcp_val}" 2>/dev/null && { _tcp_ok=$((_tcp_ok + 1)); return 0; } || true
        _tcp_proc=$(echo "$_tcp_key" | sed 's|\.|/|g')
        echo "$_tcp_val" > "/proc/sys/${_tcp_proc}" 2>/dev/null && { _tcp_ok=$((_tcp_ok + 1)); return 0; } || true
        _tcp_failed=$((_tcp_failed + 1))
    }

    _apply_tcp "net.core.rmem_max" "67108864"
    _apply_tcp "net.core.wmem_max" "67108864"
    _apply_tcp "net.ipv4.tcp_rmem" "4096 1048576 33554432"
    _apply_tcp "net.ipv4.tcp_wmem" "4096 1048576 33554432"
    _apply_tcp "net.core.netdev_max_backlog" "5000"

    [ "$_tcp_ok" -gt 0 ] && print_info "TCP 优化已启用 (${_tcp_ok}/$((_tcp_ok + _tcp_failed)) 项生效)" || \
        print_warn "TCP 优化启用失败，内核可能不支持"
}
...
show_summary() {
    ...
    echo "  ${BOLD}TCP 优化:${NC}    $(if [ "$DO_TCP" = true ]; then echo '是'; else echo '否'; fi)"
...
do_all() {
    ...
    [ "$DO_TCP" = true ] && { _tcp_val=$(cat /proc/sys/net/ipv4/tcp_rmem 2>/dev/null); [ -n "$_tcp_val" ] && echo "  ${GREEN}✓${NC} TCP 优化      已启用" || echo "  ${RED}✗${NC} TCP 优化      未生效"; } || echo "  ${YELLOW}○${NC} TCP 优化      跳过"
...
do_tcp() {
    print_info "开启 TCP 缓冲区优化..."
    grep -q "net.core.rmem_max = 67108864" /etc/sysctl.conf 2>/dev/null || echo "net.core.rmem_max = 67108864" >> /etc/sysctl.conf
    grep -q "net.core.wmem_max = 67108864" /etc/sysctl.conf 2>/dev/null || echo "net.core.wmem_max = 67108864" >> /etc/sysctl.conf
    grep -q "net.ipv4.tcp_rmem = 4096 1048576 33554432" /etc/sysctl.conf 2>/dev/null || echo "net.ipv4.tcp_rmem = 4096 1048576 33554432" >> /etc/sysctl.conf
    grep -q "net.ipv4.tcp_wmem = 4096 1048576 33554432" /etc/sysctl.conf 2>/dev/null || echo "net.ipv4.tcp_wmem = 4096 1048576 33554432" >> /etc/sysctl.conf
    grep -q "net.core.netdev_max_backlog = 5000" /etc/sysctl.conf 2>/dev/null || echo "net.core.netdev_max_backlog = 5000" >> /etc/sysctl.conf
    sysctl -p >/dev/null 2>&1
    [ "$(sysctl -n net.core.rmem_max 2>/dev/null)" = "67108864" ] && print_info "TCP 优化已启用" || \
        print_warn "TCP 优化启用失败"
}
...
do_all() {
    ...
    [ "$DO_INSTALL_TOOLS" = true ] && echo "  ${GREEN}✓${NC} 基础工具      已安装" || echo "  ${YELLOW}○${NC} 基础工具      跳过"
    [ "$DO_BBR" = true ] && { sysctl net.ipv4.tcp_congestion_control 2>/dev/null | grep -q bbr && echo "  ${GREEN}✓${NC} BBR           已启用" || echo "  ${RED}✗${NC} BBR           未生效"; } || echo "  ${YELLOW}○${NC} BBR           跳过"
    [ "$DO_TCP" = true ] && { [ "$(sysctl -n net.core.rmem_max 2>/dev/null)" = "67108864" ] && echo "  ${GREEN}✓${NC} TCP 优化      已启用" || echo "  ${RED}✗${NC} TCP 优化      未生效"; } || echo "  ${YELLOW}○${NC} TCP 优化      跳过"
    [ -n "$NEW_HOSTNAME" ] && [ "$(hostname)" = "$NEW_HOSTNAME" ] && echo "  ${GREEN}✓${NC} 主机名         $NEW_HOSTNAME" || { [ -n "$NEW_HOSTNAME" ] && echo "  ${RED}✗${NC} 主机名         未生效" || echo "  ${YELLOW}○${NC} 主机名         跳过"; }
    [ "$DO_TIMEZONE" = true ] && [ -f /etc/localtime ] && echo "  ${GREEN}✓${NC} 时区           $(readlink /etc/localtime 2>/dev/null | sed 's|.*/||' || echo '已设置')" || { [ "$DO_TIMEZONE" = true ] && echo "  ${RED}✗${NC} 时区           未设置" || echo "  ${YELLOW}○${NC} 时区           跳过"; }
    [ "$DO_SPEEDTEST" = true ] && { command -v speedtest >/dev/null 2>&1 || command -v speedtest-cli >/dev/null 2>&1; } && echo "  ${GREEN}✓${NC} Speedtest     已安装" || { [ "$DO_SPEEDTEST" = true ] && echo "  ${RED}✗${NC} Speedtest     未安装" || echo "  ${YELLOW}○${NC} Speedtest     跳过"; }
    [ "$DO_ZRAM" = true ] && swapon_list | grep -q zram && echo "  ${GREEN}✓${NC} ZRAM          已开启" || { [ "$DO_ZRAM" = true ] && echo "  ${RED}✗${NC} ZRAM          未生效" || echo "  ${YELLOW}○${NC} ZRAM          跳过"; }
